...
Такой подход может подходить для нескольких маленьких строк, но будет очень сильно замедляться.  
...
К счатью для нас **_minimum sliding range query problem_** может быть решена суммарно за O(n) time для всех окон, но 
это более сложный подход.  

Так же для решения нам потребуется массив или hashtable, чтобы запоминать какие и в каком количестве цветов сейчас 
